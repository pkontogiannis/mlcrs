...
tmpEye = eye(num_labels); % 2D array with ones in diagonial
yMatrix = tmpEye(:, y); % 10 x 5000

J = (1/m)*sum(sum( -yMatrix  .* log(h) - (1 -yMatrix).* log(1-h)));

% Regularized Cost Function
JReg = (lambda/(2*m))*(sum(sum(Theta1(:,2:end).^2)) .... 
...
d3 = (a3 - yMatrix); % 10 x 5000
d2 = (Theta2' * d3) .* [ones(1, m); sigmoidGradient(z2)]; % 26 x 5000
